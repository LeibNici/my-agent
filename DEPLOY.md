...
无容器、无构建步骤：一个 Node 进程（`engine/`，tsx 直接跑 TypeScript 源码，
无编译产物）直接对外提供 API + 静态前端（`:8000`），前面**没有**反向代理。
进程目前是手动 `nohup npm start &` 拉起的（PPID=1 的孤儿进程）——
**机器重启后不会自动恢复**，建议尽快换成下面的 systemd 单元。

```
┌─ /home/my-agent ──────────────────────────────┐
│  engine/  web/       ← 代码（git 管理）        │
│  engine/node_modules/ ← npm 依赖（不入 git）    │
│  engine/.env         ← 密钥/配置（不入 git，见下）│
...
`.env` 位置容易搞混：`agent_data.db`/`.jwt_secret`/`web/` 都是 Node 代码
自己按 `import.meta.url` 算出仓库根目录来定位的，与进程 cwd 无关；但
`.env` 是 dotenv 按**进程 cwd** 读的（见根 `CLAUDE.md` 的 Running 小节），
而 `deploy/codeaxis.service` 的 `WorkingDirectory` 是 `engine/`——所以
`.env` 必须放在 `engine/.env`，不是仓库根。
...
| **运行密钥** | `engine/.env` | **必须拷**（DashScope key、admin 引导账号等） |
...
apt install -y nodejs npm git universal-ctags ripgrep
#    universal-ctags: 符号索引（缺失则该功能自动降级）
#    ripgrep:         code_search 加速（缺失则回退 grep -F，功能不变）

# 2. 代码 + 依赖
git clone <repo-url> /home/my-agent && cd /home/my-agent/engine
npm install
...
scp old:/home/my-agent/engine/.env     /home/my-agent/engine/.env
scp old:/home/my-agent/.jwt_secret     /home/my-agent/.jwt_secret

# 4. 按新环境改 engine/.env（至少检查 APP_CORS_ORIGINS 指向新地址）
...
首次启动会自动：跑数据库建表（`initSchema` 内置，幂等 DDL，无需手动）、
克隆所有仓库、后台重建 ctags 和 embedding 索引。管理页仓库列表能看到
同步/索引状态。
...
2. **仓库目录在 /tmp**：机器重启即被清空。能自愈（自动重克隆+重建索引），
   但 embedding 全量重建有 API 费用。→ `engine/.env` 里设
