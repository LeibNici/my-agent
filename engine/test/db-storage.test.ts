...
describe("getIssueTrackingConfig / setIssueTrackingConfig（2026-07-14，修复报告信任的 bot 用户名 DB 化）", () => {
  it("没有任何行时返回 null", () => {
    expect(storage.getIssueTrackingConfig()).toBeNull();
  });

  it("setIssueTrackingConfig 首次写入并可读回", () => {
    const row = storage.setIssueTrackingConfig({ fixBotUsername: "169437" });
    expect(row.fix_bot_username).toBe("169437");
    expect(storage.getIssueTrackingConfig()).toEqual(row);
  });

  it("再次 setIssueTrackingConfig 是 UPSERT，覆盖旧值", () => {
    storage.setIssueTrackingConfig({ fixBotUsername: "old-bot" });
    storage.setIssueTrackingConfig({ fixBotUsername: "new-bot" });
    expect(storage.getIssueTrackingConfig()!.fix_bot_username).toBe("new-bot");
  });
});
...
describe("getTrackableSubmissions（v1 database.py:710 同语义：open/未知恒轮询，closed 每天至多查一次）", () => {
  ...
  it("remote_state 显式为非 'closed' 值（如 'open'）⇒ 恒被包含，不看 last_checked_at", () => {
    ...
    storage.updateIssueTracking(id, { remoteState: "open" }, storage.beginPoll(id)); // last_checked_at 被戳为刚刚
    expect(storage.getTrackableSubmissions().map((r) => r.id)).toContain(id);
  });

  it("closed 且 last_checked_at 在 1 天以内 ⇒ 被排除", () => {
    const id = seedSubmission(4);
    storage.updateIssueTracking(id, { remoteState: "closed" }, storage.beginPoll(id)); // last_checked_at 刚刚 = 现在
  ...
  it("closed 但 last_checked_at 超过 1 天 ⇒ 被重新纳入（防止错过 reopen）", () => {
    ...
    storage.updateIssueTracking(id, { remoteState: "closed" }, storage.beginPoll(id));
    const db = new Database(dbPath);
...
describe("updateIssueTracking（status_changed_at 的 CASE 防竞态写法，v1 database.py:732 同语义）", () => {
  ...
  it("传入与当前一致的 trackStatus（列默认值 'submitted'）⇒ status_changed_at 保持不变（null 到 null，逐字节相同）", () => {
    ...
    storage.updateIssueTracking(id, { trackStatus: "submitted" }, storage.beginPoll(id));
    expect(readStatusChangedAt(id)).toBeNull();
  });

  it("传入真正不同的 trackStatus ⇒ status_changed_at 被戳为新鲜时间戳；之后再传相同值就不再变化", () => {
    const id = seedSubmission();
    storage.updateIssueTracking(id, { trackStatus: "in_progress" }, storage.beginPoll(id));
  ...
  it("last_checked_at 每次调用都被戳新（哪怕不传 trackStatus，标记的是轮询尝试本身）", () => {
    const id = seedSubmission();
    storage.updateIssueTracking(id, {}, storage.beginPoll(id));
    const db = new Database(dbPath);
  ...
  it("clearError 优先于 trackError（同时传入时按 v1 的 if/elif 语义，清空而不是写入新错误文本）", () => {
    const id = seedSubmission();
    storage.updateIssueTracking(id, { trackError: "boom" }, storage.beginPoll(id));
    ...
    storage.updateIssueTracking(id, { trackError: "should be ignored", clearError: true }, storage.beginPoll(id));
    const db2 = new Database(dbPath);
    expect(
...
// webhook receiver, and an on-demand recheck can all race to update the
// same submission — beginPoll/updateIssueTracking's generation guard
// ensures whichever attempt STARTED most recently wins, regardless of
// which one's network round-trip happens to finish first.
describe("beginPoll / updateIssueTracking 的 generation 防竞态守卫（2026-07-14）", () => {
  function seedSubmission(): number {
    const uid = storage.createUser("alice", "hashed-pw");
    const repoId = storage.createRepo({ name: "r", url: "https://example.com/r.git" });
    return storage.recordIssueSubmission({
  ...
  it("beginPoll 每次调用返回递增的票号", () => {
    const id = seedSubmission();
    const g1 = storage.beginPoll(id);
    const g2 = storage.beginPoll(id);
    const g3 = storage.beginPoll(id);
    expect(g2).toBeGreaterThan(g1);
    expect(g3).toBeGreaterThan(g2);
  });

  it("旧 generation 的写入在新 generation 已经开始之后静默失败（不抛错，只是不生效）— 模拟慢 cron poll 晚于 webhook 完成", () => {
    const id = seedSubmission();
    // Simulates: cron poll starts (gets ticket 1), webhook fires and starts
    // its own poll attempt for the same submission (gets ticket 2) and
    // finishes FIRST, THEN the slow cron poll from ticket 1 finally
    // resolves and tries to write its now-stale data.
    const cronGeneration = storage.beginPoll(id); // ticket 1 — "started earlier"
    const webhookGeneration = storage.beginPoll(id); // ticket 2 — "started later"

    storage.updateIssueTracking(id, { trackStatus: "closed", remoteState: "closed" }, webhookGeneration);
    // The slow cron poll's write arrives AFTER, carrying the OLDER ticket —
    // must not overwrite what the webhook (newer ticket) already wrote.
    storage.updateIssueTracking(id, { trackStatus: "submitted", remoteState: "opened" }, cronGeneration);

    const db = new Database(dbPath);
    const row = db.prepare("SELECT track_status, remote_state FROM issue_submissions WHERE id = ?").get(id) as any;
    db.close();
    expect(row).toMatchObject({ track_status: "closed", remote_state: "closed" });
  });

  it("新 generation 的写入总是生效（正常顺序：没有并发竞争时行为不变）", () => {
    const id = seedSubmission();
    const g1 = storage.beginPoll(id);
    storage.updateIssueTracking(id, { trackStatus: "submitted" }, g1);
    const g2 = storage.beginPoll(id);
    storage.updateIssueTracking(id, { trackStatus: "closed" }, g2);

    const db = new Database(dbPath);
    const row = db.prepare("SELECT track_status FROM issue_submissions WHERE id = ?").get(id) as any;
    db.close();
    expect(row.track_status).toBe("closed");
  });

  it("崩溃的最新 attempt 不会永久卡住这一行 —— 下一次 beginPoll 拿到的新票号仍然能成功写入", () => {
    const id = seedSubmission();
    storage.beginPoll(id); // "crashed" attempt — claims a ticket, never writes
    // Self-healing: the NEXT poll attempt (of any kind) claims a fresh,
    // higher ticket for itself and its write succeeds normally.
    const recovery = storage.beginPoll(id);
    storage.updateIssueTracking(id, { trackStatus: "closed" }, recovery);
...
describe("getMyIssueSubmissions / getMyUnreadIssueCount / markMyIssuesSeen（fresh 的 null/非 null 边界，v1 database.py:835/868 同语义）", () => {
  function seedTrackedSubmission(userId: number, repoId: number, issueNumber: number): number {
    ...
    storage.updateIssueTracking(subId, { trackStatus: "in_progress" }, storage.beginPoll(subId));
    return subId;
  }
...
describe("getIssueTrackingOverview（counts 按 COALESCE(track_status,'submitted') 分组 + labels/remote_labels 真实数组，v1 database.py:892 同语义）", () => {
  ...
  it("counts 按 track_status 分组统计（未变更的默认 'submitted' 与显式变更的状态分别计数）", () => {
    ...
    storage.updateIssueTracking(id3, { trackStatus: "closed" }, storage.beginPoll(id3));
  ...
  it("labels/remote_labels 解析为真实数组；remote_labels 为 NULL 时是 []而不是抛错", () => {
    ...
    storage.updateIssueTracking(id, { remoteLabels: JSON.stringify(["needs-info"]) }, storage.beginPoll(id));
