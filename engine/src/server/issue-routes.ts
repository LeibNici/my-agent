...
export function mountIssueRoutes(app: Hono<Env>, deps: IssueRoutesDeps): void {
  app.post("/api/issues/submit", async (c) => {
    ...
    if (sessionId) {
      ...
      // (with the reporter stamp), not the unstamped draft. Also close
      // out the session: its task is done, so the next message on it
      // should start fresh rather than pile on.
      await deps.db.recordIssueSubmission({
        sessionId,
        repoId: body.repo_id,
        userId: user.id,
        title: body.title,
      ...
      await deps.db.markSessionResolved(sessionId);
    }
