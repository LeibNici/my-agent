...
## draft_issue：可验证输出设计（2026-07-15）

`tools/github-issue.ts` 的 `DRAFT_ISSUE_DESCRIPTION` 在原有五段式 body 之上
加了三条纯 prompt 层规则（不改 schema/execute()/前端），目的是把"人工确认"
从"只能盲选"变成"看得懂该核实哪一部分"：

- **受控触发的同类点扫描**：根因确认后，优先复用本轮对话里已有的同等
  全仓 `code_search` 结果；没有的话，只有当根因本身是"可复现的调用/实现
  模式"（同一符号被多处以同样错误方式调用、同一逻辑被复制）才追加**一次**
  `code_search`；孤立的配置错误/拼写错误不触发，"存在多个调用方"本身也
  不算触发理由——这条负面清单是刻意写死的，避免"模式性问题"这个词过宽
  导致每次起草都多打一轮工具调用。
- **三档互斥分类 + 候选影响范围免责声明**：扫描结果分`已确认同类问题`/
  `疑似同类问题`（归在 `同类问题排查` 小节下）/`候选影响范围/关联调用`
  （独立小节，覆盖同符号调用点或同字面量命中里"确认不复现原 bug"的部
  分），命中符号名或字面量不能直接算"已确认"；后者必须逐字带上"这是
  基于文本搜索的候选清单，不是完整调用图，不能当作详尽的影响分析。"——
  `code_search` 是字面量搜索，不是调用图，这条声明防止 body 传达"AI 已
  经做过完整影响分析"这种假象。
- **全 body 证据标注**：原本只有 `expected_behavior` 字段要求"推测的内容
  写'推测：...，请确认'"，现在这条规则扩展到代码位置/根因等 body 里的
  所有关键论断——没有工具调用实证的，必须显式标推测，不能用确定语气陈述
  未验证的猜测。用户自己描述的复现信息（工单号、操作步骤）不受此约束，
  按用户原话记录即可。
